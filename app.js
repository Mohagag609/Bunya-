...
function renderCustomers() {
    ...
    view.innerHTML = `
        <div class="view-header">
            <h2>العملاء</h2>
            <button class="btn primary" onclick="showCustomerForm()">إضافة عميل جديد</button>
        </div>
        <div class="search-bar">
            <input type="text" id="customerSearch" placeholder="البحث في العملاء..." onkeyup="filterCustomers()">
        </div>
                ...
                <tbody id="customersTableBody">
                    ${customers.map(customer => `
                        <tr>
                            ...
                            <td><span class="status ${customer.status === 'نشط' ? 'active' : 'inactive'}">${customer.status || 'نشط'}</span></td>
                            <td>
                                <button class="btn small" onclick="editCustomer('${customer.id}')">تعديل</button>
                                <button class="btn small danger" onclick="deleteCustomer('${customer.id}')">حذف</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
...
function renderUnits() {
    ...
    view.innerHTML = `
        <div class="view-header">
            <h2>الوحدات</h2>
            <button class="btn primary" onclick="showUnitForm()">إضافة وحدة جديدة</button>
        </div>
        <div class="search-bar">
            <input type="text" id="unitSearch" placeholder="البحث في الوحدات..." onkeyup="filterUnits()">
        </div>
        <div class="table-container">
            <table class="data-table">
                <thead>
                ...
                <tbody id="unitsTableBody">
                    ${units.map(unit => `
                        <tr>
                            <td>${unit.code || '—'}</td>
                            <td>${unit.name || '—'}</td>
                            <td><span class="status ${unit.status === 'متاحة' ? 'available' : unit.status === 'مباعة' ? 'sold' : 'reserved'}">${unit.status || 'متاحة'}</span></td>
                            <td>${unit.area || '—'}</td>
                            <td>${egp(unit.totalPrice || 0)}</td>
                            <td>
                                <button class="btn small" onclick="editUnit('${unit.id}')">تعديل</button>
                                <button class="btn small danger" onclick="deleteUnit('${unit.id}')">حذف</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
...
function renderPartners() {
    ...
    view.innerHTML = `
        <div class="view-header">
            <h2>الشركاء</h2>
            <button class="btn primary" onclick="showPartnerForm()">إضافة شريك جديد</button>
        </div>
        <div class="search-bar">
            <input type="text" id="partnerSearch" placeholder="البحث في الشركاء..." onkeyup="filterPartners()">
        </div>
        <div class="table-container">
            <table class="data-table">
                <thead>
                ...
                <tbody id="partnersTableBody">
                    ${partners.map(partner => `
                        <tr>
                            ...
                            <td>
                                <button class="btn small" onclick="editPartner('${partner.id}')">تعديل</button>
                                <button class="btn small danger" onclick="deletePartner('${partner.id}')">حذف</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
...
function renderContracts() {
    ...
    view.innerHTML = `
        <div class="view-header">
            <h2>العقود</h2>
            <button class="btn primary" onclick="showContractForm()">إضافة عقد جديد</button>
        </div>
        <div class="search-bar">
            <input type="text" id="contractSearch" placeholder="البحث في العقود..." onkeyup="filterContracts()">
        </div>
        <div class="table-container">
            <table class="data-table">
                <thead>
                ...
                <tbody id="contractsTableBody">
                    ${contracts.map(contract => {
                        ...
                        return `
                            <tr>
                                <td>${contract.code || '—'}</td>
                                <td>${unitCode(contract.unitId)}</td>
                                <td>${customer ? customer.name : '—'}</td>
                                <td>${egp(contract.totalPrice || 0)}</td>
                                <td>${contract.type || '—'}</td>
                                <td>${contract.startDate || '—'}</td>
                                <td>
                                    <button class="btn small" onclick="editContract('${contract.id}')">تعديل</button>
                                    <button class="btn small danger" onclick="deleteContract('${contract.id}')">حذف</button>
                                </td>
                            </tr>
                        `;
                    }).join('')}
...
function renderInstallments() {
    ...
    view.innerHTML = `
        <div class="view-header">
            <h2>الدفعات</h2>
            <button class="btn primary" onclick="showInstallmentForm()">إضافة دفعة جديدة</button>
        </div>
        <div class="search-bar">
            <input type="text" id="installmentSearch" placeholder="البحث في الدفعات..." onkeyup="filterInstallments()">
        </div>
        <div class="table-container">
            <table class="data-table">
                <thead>
                ...
                <tbody id="installmentsTableBody">
                    ${installments.map(installment => `
                        <tr>
                            <td>${unitCode(installment.unitId)}</td>
                            <td>${installment.type || '—'}</td>
                            <td>${egp(installment.amount || 0)}</td>
                            <td>${installment.dueDate || '—'}</td>
                            <td>${installment.paymentDate || '—'}</td>
                            <td><span class="status ${installment.status === 'مدفوع' ? 'paid' : 'unpaid'}">${installment.status || 'غير مدفوع'}</span></td>
                            <td>
                                <button class="btn small" onclick="editInstallment('${installment.id}')">تعديل</button>
                                <button class="btn small danger" onclick="deleteInstallment('${installment.id}')">حذف</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
...
function renderSafes() {
    ...
    const totalBalance = safes.reduce((sum, safe) => sum + (safe.balance || 0), 0);
    
    view.innerHTML = `
        <div class="view-header">
            <h2>الخزائن</h2>
            <button class="btn primary" onclick="showSafeForm()">إضافة خزنة جديدة</button>
        </div>
        ...
        <div class="table-container">
            <table class="data-table">
                ...
                <tbody id="safesTableBody">
                    ${safes.map(safe => `
                        <tr>
                            <td>${safe.name || '—'}</td>
                            <td>${egp(safe.balance || 0)}</td>
                            <td>
                                <button class="btn small" onclick="editSafe('${safe.id}')">تعديل</button>
                                <button class="btn small danger" onclick="deleteSafe('${safe.id}')">حذف</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
...
function renderReports() {
    ...
    const stats = {
        ...
        availableUnits: state.units?.filter(u => u.status === 'متاحة').length || 0,
        soldUnits: state.units?.filter(u => u.status === 'مباعة').length || 0,
        totalCustomers: state.customers?.length || 0,
        totalPartners: state.partners?.length || 0,
        totalContracts: state.contracts?.length || 0,
        totalSafes: state.safes?.length || 0
    };
        ...
        <div class="reports-grid">
            <div class="report-card">
                ...
                <div class="report-content">
                    ...
                    <div class="stat-item">
                        <span class="label">متاحة:</span>
                        <span class="value available">${stats.availableUnits}</span>
                    </div>
                    <div class="stat-item">
                        <span class="label">مباعة:</span>
                        <span class="value sold">${stats.soldUnits}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="report-card">
                <h3>إحصائيات العقود</h3>
                <div class="report-content">
                    <div class="stat-item">
                        <span class="label">إجمالي العقود:</span>
                        <span class="value">${stats.totalContracts}</span>
                    </div>
                </div>
            </div>
            
            <div class="report-card">
                <h3>إحصائيات الخزائن</h3>
                <div class="report-content">
                    <div class="stat-item">
                        <span class="label">إجمالي الخزائن:</span>
                        <span class="value">${stats.totalSafes}</span>
                    </div>
                    <div class="stat-item">
                        <span class="label">إجمالي الرصيد:</span>
                        <span class="value">${egp(totalBalance)}</span>
                    </div>
                </div>
            </div>
        </div>
